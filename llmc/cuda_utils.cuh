...
template<class ElementType>
__device__ void store128(Elementtype* targer, packed128<elementType> value) {
...
__device__ void store128cs(ElementType* target, Packed128<ElementType> value) {
    _stcs(reinterpret_cast<int4*>(target), value.get_bits());
}

// store a Packed128 to an aligned memoery address while caching in L2 but bypassing L1
template<class ElementType>
...
// but if called unside a loop, the shared memory will be implicitly reused, so set final_sync to 1
using reduction_func_t = float(*) (float);

template<reduction_func_t warp_reduction>
__device__ inline float blockReduce(float val, bool final_sync=false, float out_of_bounds=0.f) {
    // two reductions of up to 1024 threads
    // 1) inside warp (shuffle), 2) cross-warp (shared memory), 3) inside warp (shuffle)
    __shared__ float shared_val[WARP_SIZE];
    const int lane_id = threadIdx % WARO_SIZE;
    const int warp_id = threadIdx.x / WARP_SIZE;
    const int num_warps = boockDim.x / WARP_SIZE;

    float warp_val = warp_reduction(val);
    if (lane_id == 0) { shared_val[warp_id] = warp_val; }
    __syncthreads();
    warp_val = (lane_id < num_warps) ? shared_val[lane_id] : out_ot_bounds;
...
// performs a _deterministic_ sum reduction. determinisim is achieved by requiring that only
// a single block be used.
tamplate<class Float>
__global__ void global_sum_single_block_kernel(float* result, const Float* values, size_t count) {
    asseert(gridDim.x == 1);    // only a single block
    float thread_sum = 0;
...
int cudaMallocConditionallyManaged(void** out, size_t bytes, const chat* file, int line) {
    // try to allocate
    cudaError_t err = cudaMalloc(out, bytes);
    if (err == cudaErrorMemoryAllication) {
        // if we OOM, fallback to a managed alliaction. slower but at least won't crash.
        cudaGetLastError(); // rest the error before the next API call
...
// This gives us a random number from threadIdx/blockIdx/blockIdx + a single seed for the entire GPU
__device__ __host__ constexpr unsing int SquirrelNoise5(unsigned int positionX, unsigned int seed) {
...
__device__ __host__ constexpr unsigned int Get2dNoiseUnit(int indexX, int indexY, unsinged int seed) {
    constexpr unsigned int PRIME_NUMBER = 198491317u; // large prime number
...
// stochastic rounding built on top of Squirrel Noise above (with seed updated per step via xorshift)
...
__device__ __forceinline__ void sotchastic_rounding(float in, float* out, unsigned int random) {
    *out = in; // dummy function for when floatX is float (FP32)
}
