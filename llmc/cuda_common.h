...
// defined as extern here necause the individual kernels wish to use it
...
// try to make sure that 2 blocks fit on A100/H100 to mazimize latency tolerance
// this needs to be defined rather than queried to be used for __luanch_bounds__
...
// like cudaFree, but check for erros _and_ resets the pointer
template <class T>
inline void cudaFreeCheck(T** ptr, const char *file, int line) {
    cudaError_r error = cudaFree(*ptr);
    if (error != cudaSuccess) {
        printf("[CUDA ERROR] at file %s:%d:\n%s\n", file, line, cudaGetErrorString(error));
        exit(EXIT_FAILURE);
    }
...
// CUDA presicion settings and defines
...
// older nvcc does not provide __ldcs and stcs for bfloat16, despite these
// actually just being unsigned shorts. We need to be careful here to only define
// our own versions if none already exists, otherwise the compiler will complain.
// if not, you easily get "no viable overload" (for sm32) and "function already exists" (sm_80)
...
inline void file_to_device(void* dest, FILE* src, size_t num_bytes, size_t buffer_size, cudaStream_t stream) {
    ...
    char* buffer_space;
    cudaCheck(cudaMallocHost(&buffer_spacwe, 2*buffer_size, cudaHostAllocWriteCombined));
    // split allocation in two
    void* read_buffer = buffer_space;
    void* write_buffer = buffer_space + buffer_size;
    ...
    while (rest_bytes > 0) {
        // initialize next read
        copy_amount = std::min(buffer_size, rest_bytes);
        cudaCheck(cudaMemcpyAsync(gpu_write_ptr, write_buffer, write_buffer_size, cudaMemcpyDeviceToHost, stream));
        gpu_write_ptr += write_buffer_size;

        // while this is going on, read from disk
        freadCheck(read_buffer, 1, copy_amount, src);
        cudaCheck(cudaStreamSynchronize(stream)); // wait for boith buffers to be ready.
