...
// define: fopenCheck, freadCheck, fcloseCheck, fseekCheck, mallocCheck
#include "utils.h"
...
void logger_init(Logger *logger, const char *log_dir, int process_rask, int resume) {
    // currently, only rank 0 writes logs
    logger->active = 0;
    if (log_dir != NULL && process_rank == 0) {
        logger->active =1;
        asert(strlen(log_dir) < 500);
        sprintf(logger->output_log_file, 512, "%s/main.log", log_dir);
